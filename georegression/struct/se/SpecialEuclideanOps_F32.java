...
public class SpecialEuclideanOps_F32 {
	 ...
	 */
	public static Se3_F32 eulerXyz(float dx, float dy, float dz, float rotX, float rotY, float rotZ,
								   @Nullable Se3_F32 se) {
		return eulerXyz(dx, dy, dz, EulerType.XYZ,rotX, rotY, rotZ, se);
	}

	public static Se3_F32 eulerXyz(float dx, float dy, float dz,
								   EulerType type, float rotX, float rotY, float rotZ,
								   @Nullable Se3_F32 se) {
		if( se == null )
			se = new Se3_F32();
	 ...
	 * @return The transform.
	 */
	public static Se3_F32 axisXyz(float dx, float dy, float dz, float rotX, float rotY, float rotZ,
								  @Nullable Se3_F32 se) {
		if( se == null )
			se = new Se3_F32();

		float theta = (float)Math.sqrt(rotX*rotX + rotY+rotY + rotZ*rotZ);
		if( theta == 0 ) {
	...
	public static Se3_F32 quatXyz(float dx, float dy, float dz,
	...
	public static boolean isIdentical( Se3_F32 a , Se3_F32 b , float tolT , float tolR ) {
		if( (float)Math.abs(a.T.x-b.T.x) > tolT )
			return false;
		if( (float)Math.abs(a.T.y-b.T.y) > tolT )
			return false;
		if( (float)Math.abs(a.T.z-b.T.z) > tolT )
			return false;

		FMatrixRMaj D = new FMatrixRMaj(3,3);
		CommonOps_FDRM.multTransA(a.R,b.R,D);

		Rodrigues_F32 rod = new Rodrigues_F32();
		ConvertRotation3D_F32.matrixToRodrigues(D,rod);
	...
	public static boolean bestFit( Se3_F32 a ) {
		...
		b = (float)Math.signum(det) / (float)Math.pow(b,1.0f/3.0f);
