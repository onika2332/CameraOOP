 ...
 * @author Peter Abeles
 */
@Generated("georegression.struct.se.Se2_F64")
public class Se2_F32 implements SpecialEuclidean<Se2_F32> {
	...
	public void setTo(float x, float y, float yaw ) {
		this.T.setTo(x, y);
		this.c = (float)Math.cos( yaw );
		this.s = (float)Math.sin( yaw );
	}

	public void setTo(float x, float y, float cosYaw, float sinYaw ) {
		this.T.setTo(x, y);
		this.c = cosYaw;
		this.s = sinYaw;
	}
	...
	@Override
	public void setTo( Se2_F32 target ) {
		this.T.set(target.T);
		this.c = target.c;
		this.s = target.s;
	}
	...
	@Override
	public Se2_F32 concat( Se2_F32 second, @Nullable Se2_F32 result ) {
		if( result == null )
			result = new Se2_F32();
	...
	@Override
	public Se2_F32 invert( @Nullable Se2_F32 inverse ) {
		if( inverse == null )
			inverse = new Se2_F32();
